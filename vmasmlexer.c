...
char *open_file(char *file_path, int *length) {
    ...
    *length = ftell(file);
    ...
    size_t read_count = fread(current, sizeof(char), *length, file);
    if (read_count != *length) {
        fprintf(stderr, "ERROR: Incorrect reading from a file\n");
        free(current);
        fclose(file);
        exit(1);
    }
...
TokenType check_builtin_keywords(const char *name){
